...
当 $p,n\to\infty$ 且 $p/n=\gamma\le1$ 保持固定时，$S_n$ 的经验谱分布收敛到[^mp-support]
...
若 $\lambda$ 不是 $Z_n$ 的特征值，整理并左乘 $u^\top$ 得[^spike-eigenvalue]
...
[^mp-support]: Marčenko–Pastur 定律本身只排除了有非消失比例的特征值落在极限支撑之外，并不直接断言每个特征值都在支撑内；更强的极值特征值结果可参见 [@Paul_Marchenko_Pastur_BPP_TechReport]。

[^spike-eigenvalue]: 直观上，$\lambda$ 来自对 $Z_n$ 加上半正定秩一扰动 $\beta uu^\top$ 后产生的首要特征值，因此应大于 $Z_n$ 的全部特征值；严格证明超出此处讨论范围。
