...
由 Taylor 展开可验证 $\cosh x\le e^{x^2/2}$，[^cosh-taylor] 于是
...
[^cosh-taylor]: 由 $\cosh x=\sum_{n=0}^\infty x^{2n}/(2n)!$、$e^{x^2/2}=\sum_{n=0}^\infty x^{2n}/(2^n n!)$，以及 $(2n)!\ge2^n n!$，可逐项比较得到该不等式。

[^exp-tangent]: 直线 $y=1+x$ 正是曲线 $y=e^x$ 在 $x=0$ 处的切线；凸性说明曲线始终位于切线之上。
