...
::: theorem
设 $y=Ax$，其中 $x$ 为 $s$ 稀疏向量。若 $A$ 的 RIP 常数满足 $\delta_{2s}<1/3$，则 $\ell_1$ 最小化问题 [@Candes-RIP]
$$\begin{equation}
\label{l1min}
\min_z \|z\|_1, \qquad \text{满足 } Az = y = Ax
\end{equation}$$
...
::: lemma
[]{#lemma:np label="lemma:np"} 若 $x,x'\in\R^p$ 的支撑集分别包含于互不相交的 $S,S'\subseteq[p]$，且 $|S|\leq s$、$|S'|\leq s'$，则 [@Candes-RIP]
...
又因为假设 $\delta_{2s}<1/3$，
$$\begin{equation}
\frac{\delta_{2s}}{1-\delta_s} < \frac{\delta_{2s}}{1-\delta_{2s}} < \frac{1}{2} \quad \text{当 $\delta_{2s} < \frac{1}{3}$}.
\label{eq:delta}
\end{equation}$$
...
*证明.* 直接计算得到
$$\begin{equation*}
\trace (S) = \|S^{\frac{1}{2}}\|_F^2 =\sum_{k=1}^{p} \| a_k\|^2 = p, \,\, \text{且}\,
...
$$\begin{equation*}
                \hat{x}:= \mathrm{argmin}\, \|z\|_1 \quad \text{满足} \quad \Phi z=\Phi x
\end{equation*}$$
的解满足
$$\begin{equation*}
